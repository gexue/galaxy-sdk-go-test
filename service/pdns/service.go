...
// Pdns provides the API operation methods for making requests to
// pdns. See this package's package overview docs
// for details on the service.
//
// Pdns methods are safe to use concurrently. It is not safe to
...
const (
	ServiceName = "pdns"      // Name of service.
	EndpointsID = ServiceName // ID to lookup a service endpoint with.
	ServiceID   = "pdns"      // ServiceID is a unique identifer of a specific service.
)

// New creates a new instance of the Pdns client with a session.
// If additional configuration is needed for the client instance use the optional
// ksc.Config parameter to add your extra config.
//
// Example:
//     // Create a Pdns client from just a session.
//     svc := pdns.New(mySession)
//
//     // Create a Pdns client with additional configuration
//     svc := pdns.New(mySession, aws.NewConfig().WithRegion("us-west-2"))
...
func ExtraNew(info *utils.UrlInfo, p client.ConfigProvider, cfgs ...*aws.Config) *Pdns {
	c := p.ClientConfig(EndpointsID, cfgs...)
...
func SdkNew(p client.ConfigProvider, cfgs *ksc.Config, info ...*utils.UrlInfo) *Pdns {
	_info := utils.UrlInfo{}
...
func newClient(cfg aws.Config, handlers request.Handlers, endpoint, signingRegion, signingName string) *Pdns {
	svc := &Pdns{
		Client: client.New(
			cfg,
			metadata.ClientInfo{
...
// newRequest creates a new request for a Pdns operation and runs any
// custom request initialization.
func (c *Pdns) newRequest(op *request.Operation, params, data interface{}) *request.Request {

...
// Monitorv5 provides the API operation methods for making requests to
// monitorv5. See this package's package overview docs
// for details on the service.
//
// Monitorv5 methods are safe to use concurrently. It is not safe to
// modify mutate any of the struct's properties though.
type Monitorv5 struct {
	*client.Client
}
...
const (
	ServiceName = "monitor-2023-07-12" // Name of service.
	EndpointsID = "monitor"            // ID to lookup a service endpoint with.
	ServiceID   = "monitor-2023-07-12" // ServiceID is a unique identifer of a specific service.
)

// New creates a new instance of the Monitorv5 client with a session.
// If additional configuration is needed for the client instance use the optional
// ksc.Config parameter to add your extra config.
//
// Example:
//     // Create a Monitorv5 client from just a session.
//     svc := monitorv5.New(mySession)
//
//     // Create a Monitorv5 client with additional configuration
//     svc := monitorv5.New(mySession, aws.NewConfig().WithRegion("us-west-2"))
...
func ExtraNew(info *utils.UrlInfo, p client.ConfigProvider, cfgs ...*aws.Config) *Monitorv5 {
	c := p.ClientConfig(EndpointsID, cfgs...)
...
func SdkNew(p client.ConfigProvider, cfgs *ksc.Config, info ...*utils.UrlInfo) *Monitorv5 {
	_info := utils.UrlInfo{
		UseSSL:      false,
		Locate:      false,
		UseInternal: false,
...
func newClient(cfg aws.Config, handlers request.Handlers, endpoint, signingRegion, signingName string) *Monitorv5 {
	svc := &Monitorv5{
		Client: client.New(
			cfg,
			metadata.ClientInfo{
...
// newRequest creates a new request for a Monitorv5 operation and runs any
// custom request initialization.
func (c *Monitorv5) newRequest(op *request.Operation, params, data interface{}) *request.Request {

...
// Cdnv3 provides the API operation methods for making requests to
// cdnv3. See this package's package overview docs
// for details on the service.
//
// Cdnv3 methods are safe to use concurrently. It is not safe to
...
// New creates a new instance of the Cdnv3 client with a session.
// If additional configuration is needed for the client instance use the optional
// ksc.Config parameter to add your extra config.
//
// Example:
//
//	// Create a Cdnv3 client from just a session.
//	svc := cdnv3.New(mySession)
//
//	// Create a Cdnv3 client with additional configuration
//	svc := cdnv3.New(mySession, aws.NewConfig().WithRegion("us-west-2"))
...
func ExtraNew(info *utils.UrlInfo, p client.ConfigProvider, cfgs ...*aws.Config) *Cdnv3 {
	c := p.ClientConfig(EndpointsID, cfgs...)
...
func SdkNew(p client.ConfigProvider, cfgs *ksc.Config, info ...*utils.UrlInfo) *Cdnv3 {
	_info := utils.UrlInfo{}
...
func newClient(cfg aws.Config, handlers request.Handlers, endpoint, signingRegion, signingName string) *Cdnv3 {
	svc := &Cdnv3{
		Client: client.New(
			cfg,
			metadata.ClientInfo{
...
// newRequest creates a new request for a Cdnv3 operation and runs any
// custom request initialization.
func (c *Cdnv3) newRequest(op *request.Operation, params, data interface{}) *request.Request {

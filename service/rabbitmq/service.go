...
// Rabbitmq provides the API operation methods for making requests to
// rabbitmq. See this package's package overview docs
// for details on the service.
//
// Rabbitmq methods are safe to use concurrently. It is not safe to
...
// New creates a new instance of the Rabbitmq client with a session.
// If additional configuration is needed for the client instance use the optional
// ksc.Config parameter to add your extra config.
//
// Example:
//     // Create a Rabbitmq client from just a session.
//     svc := rabbitmq.New(mySession)
//
//     // Create a Rabbitmq client with additional configuration
//     svc := rabbitmq.New(mySession, aws.NewConfig().WithRegion("us-west-2"))
...
func ExtraNew(info *utils.UrlInfo, p client.ConfigProvider, cfgs ...*aws.Config) *Rabbitmq {
	c := p.ClientConfig(EndpointsID, cfgs...)
...
func SdkNew(p client.ConfigProvider, cfgs *ksc.Config, info ...*utils.UrlInfo) *Rabbitmq {
	_info := utils.UrlInfo{}
...
func newClient(cfg aws.Config, handlers request.Handlers, endpoint, signingRegion, signingName string) *Rabbitmq {
	svc := &Rabbitmq{
		Client: client.New(
			cfg,
			metadata.ClientInfo{
...
// newRequest creates a new request for a Rabbitmq operation and runs any
// custom request initialization.
func (c *Rabbitmq) newRequest(op *request.Operation, params, data interface{}) *request.Request {

...
// Cdnv1 provides the API operation methods for making requests to
// cdnv1. See this package's package overview docs
// for details on the service.
//
// Cdnv1 methods are safe to use concurrently. It is not safe to
...
// New creates a new instance of the Cdnv1 client with a session.
// If additional configuration is needed for the client instance use the optional
// ksc.Config parameter to add your extra config.
//
// Example:
//     // Create a Cdnv1 client from just a session.
//     svc := cdnv1.New(mySession)
//
//     // Create a Cdnv1 client with additional configuration
//     svc := cdnv1.New(mySession, aws.NewConfig().WithRegion("us-west-2"))
func New(p client.ConfigProvider, cfgs ...*aws.Config) *Cdnv1 {
...
func ExtraNew(info *utils.UrlInfo, p client.ConfigProvider, cfgs ...*aws.Config) *Cdnv1 {
	c := p.ClientConfig(EndpointsID, cfgs...)
...
func newClient(cfg aws.Config, handlers request.Handlers, endpoint, signingRegion, signingName string) *Cdnv1 {
	svc := &Cdnv1{
		Client: client.New(
			cfg,
			metadata.ClientInfo{
...
// newRequest creates a new request for a Cdnv1 operation and runs any
// custom request initialization.
func (c *Cdnv1) newRequest(op *request.Operation, params, data interface{}) *request.Request {

...
// Tag provides the API operation methods for making requests to
// tag. See this package's package overview docs
// for details on the service.
//
// Tag methods are safe to use concurrently. It is not safe to
...
// New creates a new instance of the Tag client with a session.
// If additional configuration is needed for the client instance use the optional
// ksc.Config parameter to add your extra config.
//
// Example:
//     // Create a Tag client from just a session.
//     svc := tag.New(mySession)
//
//     // Create a Tag client with additional configuration
//     svc := tag.New(mySession, aws.NewConfig().WithRegion("us-west-2"))
...
func ExtraNew(info *utils.UrlInfo, p client.ConfigProvider, cfgs ...*aws.Config) *Tag {
	c := p.ClientConfig(EndpointsID, cfgs...)
...
func SdkNew(p client.ConfigProvider, cfgs *ksc.Config, info ...*utils.UrlInfo) *Tag {
	_info := utils.UrlInfo{}
...
func newClient(cfg aws.Config, handlers request.Handlers, endpoint, signingRegion, signingName string) *Tag {
	svc := &Tag{
		Client: client.New(
			cfg,
			metadata.ClientInfo{
...
// newRequest creates a new request for a Tag operation and runs any
// custom request initialization.
func (c *Tag) newRequest(op *request.Operation, params, data interface{}) *request.Request {

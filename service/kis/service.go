...
// Kis provides the API operation methods for making requests to
// kis. See this package's package overview docs
// for details on the service.
//
// Kis methods are safe to use concurrently. It is not safe to
...
// New creates a new instance of the Kis client with a session.
// If additional configuration is needed for the client instance use the optional
// ksc.Config parameter to add your extra config.
//
// Example:
//     // Create a Kis client from just a session.
//     svc := kis.New(mySession)
//
//     // Create a Kis client with additional configuration
//     svc := kis.New(mySession, aws.NewConfig().WithRegion("us-west-2"))
...
func ExtraNew(info *utils.UrlInfo, p client.ConfigProvider, cfgs ...*aws.Config) *Kis {
	c := p.ClientConfig(EndpointsID, cfgs...)
...
func SdkNew(p client.ConfigProvider, cfgs *ksc.Config, info ...*utils.UrlInfo) *Kis {
	_info := utils.UrlInfo{}
...
func newClient(cfg aws.Config, handlers request.Handlers, endpoint, signingRegion, signingName string) *Kis {
	svc := &Kis{
		Client: client.New(
			cfg,
			metadata.ClientInfo{
...
// newRequest creates a new request for a Kis operation and runs any
// custom request initialization.
func (c *Kis) newRequest(op *request.Operation, params, data interface{}) *request.Request {
